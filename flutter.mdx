...
class HomeScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      body: Stack(
        children: [
          YourExistingApp(),   // your app content
          const CrowWidget(),  // floating chat button, bottom-right
        ],
      ),
    );
  }
...
`CrowWidget` renders a floating black circle button in the bottom-right corner. Tapping it slides up a chat sheet at 85% screen height with the full `CrowChatView` inside.
