...
The `@usecrow/react-native` package provides React hooks and context providers for building chat interfaces in React Native apps. You build your own native UI while the SDK handles streaming, authentication, tools, and conversation persistence.

<Note>
Unlike the web widget which ships a pre-built UI, React Native requires you to build the chat interface with your own components. The SDK provides the headless logic — you provide the native UI.
</Note>
...
Wrap your app in `CrowProvider` and use the `useChat` hook:

```tsx
import { CrowProvider, useChat, createAsyncStorageAdapter } from '@usecrow/react-native';
import AsyncStorage from '@react-native-async-storage/async-storage';

// Create storage adapter for persistence
